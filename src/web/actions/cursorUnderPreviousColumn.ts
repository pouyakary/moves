...
export async function cursorUnderPreviousColumnAbove() {
  await moveCursorUnderPreviousColumn(
    kit.columns.getPreviousRenderColumnAbove(),
  );
}

export async function cursorUnderPreviousColumnBelow() {
  await moveCursorUnderPreviousColumn(
    kit.columns.getPreviousRenderColumnBelow(),
  );
}
...
function isSpaceBetweenTwoColumnsErasable(current: number, previous: number) {
  const textBetweenTwoColumns = kit.document
    .getCurrentLineContent()
    .substring(previous, current);
